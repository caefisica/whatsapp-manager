...
async function insertLog(tableName, logData, mapLogData) {
    ...
    const { data, error } = await supabase.from(tableName).insert([mappedData]);
...
async function getLibraryAttendanceStatus() {
    try {
        ...
        if (openActions && openActions.length > 0) {
            ...
            const { data: librarianData, error: librarianError } =
                await supabase
                    .from("librarians")
                    .select("fullName")
            ...
            if (librarianData) {
                ...
                console.error(
                    "No se encontró ningún librarian para el número:",
                    managerNum
                );
            }
        ...
        if (openActions && openActions.length > 0) {
            if (
                ...
                openActions[0].timestamp > closeActions[0].timestamp
            ) {
...
async function getTodayImageDescriptions() {
    ...
    if (attendanceError) {
        console.error(
            "Error en la tabla 'libraryAttendance':",
            attendanceError
        );
        throw new Error(
            "Ha ocurrido un error al obtener los datos de la tabla 'libraryAttendance'."
        );
    }
    ...
    for (const log of attendanceData) {
        ...
        if (librarianError) {
            ...
            throw new Error(
                "Ha ocurrido un error al obtener los datos de la tabla 'librarians'."
            );
        }
