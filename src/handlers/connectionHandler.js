...
async function handleReconnection(reason) {
    ...
    switch (reason) {
        case DisconnectReason.badSession:
            console.log(
                "[PROBLEMA] Archivo de sesión corrupto, por favor elimine la sesión y escanee de nuevo."
            );
        ...
        case DisconnectReason.timedOut:
            if (retryCount < MAX_RETRIES) {
                const delay =
                    Math.pow(RECONNECTION_DELAY_MULTIPLIER, retryCount) *
                    RECONNECTION_DELAY_BASE;
                logAndReconnect(
                    `[PROBLEMA] Desconectado: ${reason}. Reconectando...`,
                    delay
                );
                retryCount++;
            } else {
                console.error(
                    "[PROBLEMA] Se ha superado el número máximo de reintentos. Reinicia el bot manualmente"
                );
            }
            break;
        default:
...
function handleConnectionUpdate(sock) {
    ...
    return async update => {
        ...
        try {
            const handler =
                connectionHandlers[connection] || handleUnknownConnectionState;
